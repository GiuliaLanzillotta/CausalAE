...
def compute_beta_vae_sklearn(dataset:DisentanglementDataset,
  ...
  """
    ...
    train_points, train_labels = _generate_training_batch(dataset, representation_function, batch_size, num_train, device)
    ...
    eval_points, eval_labels = _generate_training_batch(dataset, representation_function, batch_size, num_eval, device)
...
def _generate_training_batch(dataset:DisentanglementDataset,
  ...
  """
    ...
    for i in range(num_points):
        #TODO: parallelise this call
        labels[i], feature_vector = _generate_training_sample(dataset, representation_function, batch_size, device)
        if points is None:
...
def _generate_training_sample(dataset:DisentanglementDataset,
  ...
  """
    ...
    with torch.no_grad():
        representation1 = representation_function(observations1.to(device))
        representation2 = representation_function(observations2.to(device))
