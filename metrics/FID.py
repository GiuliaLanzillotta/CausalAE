""" Implementation of the FID scoring function
...
class FIDScorer(object):
    """ Wrapper for FID scoring functions to be used to compute FID scoring online
    (at the end of each validation step, without writing files to path)"""
    def __init__(self, dims=2048):
        super(FIDScorer, self).__init__()
        self.dims = dims
    ...
    def get_activations(self, original, fake):
        ...
        if fake.size(2) != 1 or fake.size(3) != 1:
            ...
            original = adaptive_avg_pool2d(original, output_size=(1, 1))
        fake = fake.squeeze(3).squeeze(2).cpu().numpy()
        original = original.squeeze(3).squeeze(2).cpu().numpy()

        self.generated[self.start_idx:self.start_idx + fake.shape[0]] = fake
        self.originals[self.start_idx:self.start_idx + original.shape[0]] = original
        self.start_idx += fake.shape[0]
    ...
    def calculate_activation_statistics(self):
        ...
        Note: to be called after get_activations has been called on the whole dataet.
        Returns:
